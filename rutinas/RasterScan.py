...
Purpose
-------
Llevar el escaneo de la potencia de salida de un puerto de un DUT
...
x_center = x2.get_actual_position()
...
for i, x in enumerate(np.arange(x0, x1 + step, step)):

    print(f"\n===== Profile {i} =====")

    # ** Llevar a x a la posicion de interes para hacer el profile en z

    x2.move_absolute(x)
    x2.wait_until_complete()
    ...
    header = (
        ...
        f"Scan axis: x2\n"
        f"Profile axis: z2\n"
        f"Profile range: 50 um\n"
        f"Profile speed: 25 um/s\n"
        "\n"
